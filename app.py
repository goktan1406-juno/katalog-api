...
def draw_card(cv, x, y, cw, ch, product):
    ...
    cy=y-PAD
    cv.setFillColor(DARK); cv.setFont('SansBold',9)
    for ln in wrap(cv,product['name'],'SansBold',9,LW)[:2]:
        cv.drawString(LX,cy,ln); cy-=5.5*mm
    cy-=1*mm

    cv.setFillColor(LGRAY)
    bw=tw(cv,product['ref'],'Sans',6.5)+5*mm
    ...
    if product.get('claim'):
        cv.setFillColor(RED); cv.setFont('SansObl',7.5)
        claim=str(product['claim'])
        while tw(cv,claim,'SansObl',7.5)>LW and len(claim)>5: claim=claim[:-4]+'...'
    ...
    for title,detail in product['benefits']:
        ...
        cv.setFillColor(RED); cv.setFont('SansBold',9); cv.drawString(LX,cy-4.5*mm+1.5*mm,'•')
        cv.setFillColor(DARK); cv.setFont('SansBold',7.5)
        t=str(title)
        while tw(cv,t,'SansBold',7.5)>LW-5*mm and len(t)>5: t=t[:-2]+'.'
        cv.drawString(LX+4.5*mm,cy-4.5*mm+1.5*mm,t); cy-=4.5*mm
        if detail:
            cv.setFillColor(DGRAY); cv.setFont('Sans',7.0)
            for dl in wrap(cv,str(detail),'Sans',7.0,LW-4.5*mm)[:2]:
    ...
    cv.setFillColor(DGRAY); cv.setFont('Sans',6)
    cv.drawString(LX+3*mm,BOT+4.5*mm,f"Kutu İçeriği: {str(product['name']).split(',')[0]} — {product['ref']}")
...
def build_catalog(products, output_path, category='GENEL'):
    ...
    cv.setTitle(f'TEFAL {category} Kataloğu 2024')
...
def katalog():
    try:
        ...
        if not payload or 'files' not in payload:
            return jsonify({'error': 'files alanı eksik'}), 400
